-- tdigest_percentile_of() used to interpolate between two centroids by
-- computing the slope of the line connecting their means:
--
--     m = (curr->mean - prev->mean) / (curr->count / 2.0 + prev->count / 2.0);
--     x = (value - prev->mean) / m;
--
-- Both the difference of the means and the slope could leave the float8
-- range even though every input was an ordinary finite number:
--
...
--
-- Both quantile functions now use convex combinations. The inverse function
-- computes the relative position between means without dividing by a slope,
-- and halves the means and probe when their difference would overflow.
--
-- For finite probes the result must be a probability in [0, 1]. Power-of-two
-- rescaling without input precision loss should agree within rounding error.
...
-- Every result has to be a probability. The old implementation returned NaN
-- when the difference overflowed or Infinity when the slope underflowed.
WITH q AS (
    SELECT id, descr, probes, tdigest_percentile_of(tdigest(d), probes) AS res
...
-- apart.
WITH q AS (
    SELECT tdigest_percentile_of(tdigest(v, 10000), ARRAY[-1e307, 0.0, 1e307]::double precision[]) AS res
...
WITH q AS (
    SELECT tdigest_percentile_of(tdigest(v, c, 10000), ARRAY[-1e307, 0.0, 1e307]::double precision[]) AS res
...
WITH s(k) AS (
    ...
    SELECT s.k, tdigest_percentile_of(tdigest(t.v * s.k, t.c, 10000),
