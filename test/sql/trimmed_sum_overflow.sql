...
-- (mean * count) for all the centroids in the range, and the trimmed average
-- was then calculated by dividing that sum by the number of items.
--
-- But the sum may overflow to infinity even when the average is perfectly
-- representable. The average of 1e307 values is 1e307, yet the sum of a
-- hundred of them is not a float8. The average was then reported as
-- Infinity, which is simply a wrong answer - the average of a digest is
-- always within the range of the centroid means, so it can never overflow.
--
-- And once one partial sum saturates to +Infinity and another one to
-- -Infinity, the accumulator turns into NaN, so a digest with large values
-- of both signs reported NaN for both the sum and the average.
--
-- The sum, unlike the average, may legitimately exceed the float8 range.
-- Report that as an error, just like the regular float8 arithmetic does,
...
--
-- This affects tdigest_sum() and tdigest_avg(), i.e. anything going through
-- tdigest_trimmed_agg().
...
-- The same thing with the digest built by an aggregate rather than parsed from
-- a literal, first with an explicit count and then value by value.
